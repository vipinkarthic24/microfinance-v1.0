    ...
    with get_db() as conn:
        ...
        if request.loan_type == "personal_loan":
            if cibil < 600:
                return {
                    "status": "rejected",
                    "message": f"Loan rejected due to low CIBIL score ({cibil})"
                }
        ...
        # ================= GOLD LOAN =================
            ...
            if request.amount > max_loan:
                raise HTTPException(
                    status_code=400,
                    detail=f"Loan amount exceeds 70% of gold value. Max: ₹{round(max_loan,2)}"
                )
        ...
        # 4️⃣ Fetch interest rate
        rate_row = cursor.fetchone()
        if not rate_row:
            raise HTTPException(
                status_code=400,
                detail="Interest rate not configured"
            )
        ...
        cursor.execute(
            ...
            """,
            (
                loan_id,customer["id"],request.loan_type,request.amount,request.tenure,interest_rate,emi_amount,processing_fee,disbursed_amount,
                request.amount,loan_status,request.vehicle_age,request.gold_weight,branch_id,tenant_id,datetime.now(timezone.utc).isoformat()
            )
        )
        # 8️⃣ Audit
        log_audit(conn,token_data["user_id"],token_data['tenant_id'],"LOAN_APPLICATION","loan",loan_id,json.dumps({"status": loan_status}))
...
async def export_gold_rate_pdf(
    ...
    # 🏢 FETCH COMPANY + BRANCH DYNAMICALLY
    with get_db() as conn:
        cursor = conn.cursor()

        # ⭐ Company name (tenant table)
        cursor.execute(
            "SELECT name FROM tenants WHERE id = ?",
            (token_data["tenant_id"],)
        )
        ...
        # ⭐ Branch name based on logged user
       ...
       # ⭐ FETCH ONLY LATEST RATE PER DAY
        cursor.execute("""
            ...
            AND updated_at = (
                ...
                FROM gold_rate g2
                WHERE date(g2.updated_at) = date(g1.updated_at)
            )
    ...
    # 📊 REPORT TITLE
    elements.append(Paragraph("Gold Rate History Report", styles['Title']))
    ...
    headers={
        "Content-Disposition": "attachment; filename=gold_rate_report.pdf"
    }
)
